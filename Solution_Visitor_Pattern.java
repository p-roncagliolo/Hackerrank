...
class ProductOfRedNodesVisitor extends TreeVis {
    ...
    private final int M = 1000000007; // NOVAAAA
...
public class Solution_Visitor_Pattern {
    ...
    public static Tree solve() {
        ...
        for (int i = 0; i < numNodes - 1; i++) {
            ...
            HashSet<Integer> uNeighbors = nodesMap.get(u);
            if (uNeighbors == null) {
                uNeighbors = new HashSet();
                nodesMap.put(u, uNeighbors);
            }
            uNeighbors.add(v);

            /* Edges are undirected: Add 2nd direction */ //NOVAAAA
            HashSet<Integer> vNeighbors = nodesMap.get(v);
            if (vNeighbors == null) {
                vNeighbors = new HashSet();
                nodesMap.put(v, vNeighbors);
            }
            vNeighbors.add(u);
        }
        scan.close();
    ...
    private static void addChildren(TreeNode parent, Integer parentNum) {
        ...
        for (Integer treeNum : nodesMap.get(parentNum)) {
            nodesMap.get(treeNum).remove(parentNum); // removes the opposite arrow direction //NOVAAAAA

            /* Add child */
            HashSet<Integer> grandChildren = nodesMap.get(treeNum);
